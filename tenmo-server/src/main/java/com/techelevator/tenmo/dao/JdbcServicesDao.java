...
public class JdbcServicesDao implements ServicesDao{
    ...
    public void pushTransfer(int id) {
        ...
        String sql = "START TRANSACTION;" +
                    ...
                    "COMMIT;";
        try {
            jdbcTemplate.queryForRowSet(sql, transfer.getAmount(), transfer.getAccountFromId(), transfer.getAmount(), transfer.getAccountToId());
        } catch (CannotGetJdbcConnectionException e) {
            throw new DaoException("Cannot connect ot server or database", e);
        } catch (DataIntegrityViolationException e) {
            throw new DaoException("Data Integrity Violation", e);
        }
    }
